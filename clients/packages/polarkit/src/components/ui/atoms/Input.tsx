  ...
  ({ preSlot, postSlot, className, ...props }: InputProps, ref) => {
    return (
      <div className="relative flex flex-1 flex-row">
        ...
        />
        {preSlot && (
          <div className="dark:text-polar-400 pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-gray-600">
            {preSlot}
          </div>
        )}
        {postSlot && (
          <div className="dark:text-polar-400 pointer-events-none absolute inset-y-0 right-0 flex items-center pr-4 text-gray-600">
            {postSlot}
          </div>
        )}
      </div>
