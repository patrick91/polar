...
const ProgressText = ({
  ...
  return (
    <p className="dark:text-polar-400 mt-0.5 text-xs text-gray-600">
...
const EmbedSetting = ({
  ...
  const getTabClasses = (active: boolean) => {
    return twMerge(
      active
        ? 'bg-gray-50 rounded-lg text-gray-900 shadow dark:bg-polar-600 dark:text-polar-50'
        : '',
  ...
  return (
    <>
      <div className="dark:bg-polar-900 dark:text-polar-400 flex flex-row rounded-lg bg-gray-200 text-sm text-gray-600">
        <div
    ...
    <div
      className={twMerge(
        showControls && repo.is_private
          ? 'dark:bg-polar-900 bg-gray-200/50 py-2 text-sm'
        ...
        <div className="w-full text-right">
          ...
          {showControls && (
            <div className="flex flex-row justify-end space-x-2 align-middle">
              {repo.is_private && (
                <p className="dark:bg-polar-800 dark:border-polar-700 dark:text-polar-400 flex items-center rounded-full border bg-gray-200 px-3 py-1 text-xs text-gray-600">
                  Private
                </p>
              )}
              {!repo.is_private && (
